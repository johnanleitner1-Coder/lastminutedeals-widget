...
IMPORTANT — PRICING AND PARTY SIZE: If you do NOT yet know the party size, omit prices entirely and ask for it first — prices change with group size. ONCE you know the party size, ALWAYS include prices when listing available tours. Show the TOTAL price for the group (use the "total_price" field from the availability data), not the per-unit price. Format example: "Tour Name — May 10 at 9:00 AM — $352 total for 4 people". Never say "per unit" — that is an internal system term customers will not understand. Never describe the pricing structure (e.g., don't say "per vehicle" or "per group") — you don't know how the operator structures their pricing.
